  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <BackButton variant="outline" onClick={onBack} label="Quay về Crops" />
        <div>
          <h2>Standard Tasks</h2>
          <p className="text-sm text-muted-foreground">
            {crop.cropType} {crop.variety && `- ${crop.variety}`}{" "}
            {crop.currentStage && `· Current Stage: ${crop.currentStage}`}
          </p>
        </div>
      </div>
        ...
        <CardContent>
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-sm text-muted-foreground max-w-md">
              Standard tasks will be available when connected to agronomy task
              API. Get proven recommendations for each growth stage.
            </p>
          </div>
        </CardContent>
      </Card>
