...
export function FarmsListPage() {
      ...
      <PageContainer variant="wide" className="flex items-center justify-center">
        <div className="flex flex-col items-center gap-4 max-w-md text-center">
          ...
          <h2 className="text-2xl font-bold text-foreground mb-2">
            {t("farmManagement.errorTitle")}
          </h2>
          <p className="text-muted-foreground mb-6">
            {error?.message || t("farmManagement.errorDescription")}
          </p>
  ...
  const editingFarm = editingFarmId
    ? farms.find((farm) => farm.id === editingFarmId)
    : undefined;
            ...
            <div className="bg-card rounded-xl border border-border shadow-sm p-12">
              <div className="flex flex-col items-center text-center">
                ...
                <h3 className="text-lg font-semibold text-foreground mb-2">
                  {t("farmManagement.noResultsTitle")}
                </h3>
                <p className="text-muted-foreground mb-4">
                  {t("farmManagement.noResultsDescription")}
                </p>
                <Button variant="outline" onClick={handleClearFilters}>
                  {t("farmManagement.clearFilters")}
                </Button>
              </div>
            </div>
          ) : (
