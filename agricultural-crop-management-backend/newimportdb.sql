...
-- Additional non-baseline users for marketplace split-seller and status demos.
INSERT INTO users (user_name, email, phone, full_name, password_hash, status, province_id, ward_id, joined_date)
SELECT 'farmer2', 'farmer2@acm.local', '0904234567', 'Le Thi Farmer 2',
...
INSERT INTO users (user_name, email, phone, full_name, password_hash, status, province_id, ward_id, joined_date)
SELECT 'employee2', 'employee2@acm.local', '0903234567', 'Tran Thi Employee',
       '$2a$10$BzROX8TgxrKpb./sQD9w..VmxFh1AJjAQAH8mxhJfdmpb2C7aWLIy',
...
INSERT INTO users (user_name, email, phone, full_name, password_hash, status, province_id, ward_id, joined_date)
SELECT 'buyer_locked', 'buyer.locked@acm.local', '0905555001', 'Pham Van Locked Buyer',
       '$2a$10$BzROX8TgxrKpb./sQD9w..VmxFh1AJjAQAH8mxhJfdmpb2C7aWLIy',
...
SET @rice_crop_id := (SELECT crop_id FROM crops WHERE crop_name IN ('Rice', 'Lúa nước') ORDER BY crop_id LIMIT 1);
...
SELECT '2026 - Vụ Lúa Hè Demo', @plot_id, @rice_crop_id, @rice_variety_id,
       ...
       220000, 218000, 3300.00, NULL, 16500000.00, 'Vụ demo tương thích Flyway', @seed_now
...
INSERT INTO seasons (season_name, plot_id, crop_id, variety_id, start_date, planned_harvest_date, end_date, status,
                     initial_plant_count, current_plant_count, expected_yield_kg, actual_yield_kg, budget_amount, notes, created_at)
SELECT '2026 - Vụ Ngô Cao Nguyên Demo', @plot2_id, @rice_crop_id, @rice_variety_id,
       '2026-03-01', '2026-06-15', '2026-06-20', 'COMPLETED',
       18000, 17500, 2600.00, 2650.00, 8000000.00, 'Vụ demo cho split seller marketplace', @seed_now
...
SELECT @season2_id, '2026-06-20', 940.00, 'kg', 'A', 'Thu hoạch demo cho marketplace', @seed_now
WHERE @season2_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM harvests WHERE season_id = @season2_id AND harvest_date = '2026-06-20' AND note = 'Thu hoạch demo cho marketplace');

SET @harvest2_id := (SELECT harvest_id FROM harvests WHERE season_id = @season2_id AND harvest_date = '2026-06-20' AND note = 'Thu hoạch demo cho marketplace' LIMIT 1);

INSERT INTO suppliers (name, license_no, contact_email, contact_phone)
SELECT 'Công ty TNHH Vật tư Nông nghiệp Xanh', 'BN-DEMO-001', 'vattuxanh@example.com', '02743855666'
...
SELECT 'Quy trình demo canh tác lúa', 'https://example.com/docs/rice-demo.pdf', 'Tài liệu demo sau Flyway.',
       'Rice', 'ALL', 'GUIDE', TRUE, TRUE, @admin_user_id, 'GUIDE', 0, TRUE, @seed_now, @seed_now
WHERE @admin_user_id IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM documents WHERE title = 'Quy trình demo canh tác lúa');

SET @document_id := (SELECT document_id FROM documents WHERE title = 'Quy trình demo canh tác lúa' LIMIT 1);
...
SELECT @season_id, @plot_id, '2026-05-02', 7.5000, 1.5000, NULL, 1100.0000,
       NULL, NULL, FALSE, TRUE, 'LAB_MEASURED', 'https://example.com/lab/irrigation-demo.pdf',
...
SELECT @season_id, @plot_id, '2026-05-03', 3.2000, 11.0000, 14.0000, 4.0000,
       NULL, NULL, FALSE, TRUE, 'LAB_MEASURED', 'https://example.com/lab/soil-demo.pdf',
...
SELECT @task_id, @employee_user_id, 75, 'Cập nhật tiến độ demo', 'https://example.com/evidence/task-demo-progress.jpg', @seed_now
...
SELECT 'LOT-CORN-DEMO-2026', 5001, 'Ngo ngot Cao Nguyen Xanh', 'Loai A', @season2_id, @farm2_id, @plot2_id, @harvest2_id, @output_warehouse_id, @output_location_id,
       '2026-06-20', '2026-06-20 18:00:00', 'kg', 940.000, 900.000, 'A', 'FRESH',
       '{"route":"demo-plot->output-warehouse"}', 'Lô demo cho marketplace', 'IN_STOCK', @farmer2_user_id, @seed_now, @seed_now
WHERE @season2_id IS NOT NULL
...
SELECT 'ngo-ngot-cao-nguyen-xanh-demo', 'Ngo ngot Cao Nguyen Xanh Demo', 'CORN',
       'Published listing owned by second seller.', 'Used for split-order and buyer catalog demo.', 170000.00, 'kg', 900.000,
       'https://example.com/images/corn-demo.jpg', '["https://example.com/images/corn-demo.jpg"]',
...
SELECT @buyer_user_id, 'Tran Thi Buyer', '0903234000', 'Đồng Tháp', 'Cao Lãnh', 'Mỹ An', '123 Đường Demo', 'Căn góc, gần trường học', 'home', TRUE, @seed_now, @seed_now
WHERE @buyer_user_id IS NOT NULL
...
SELECT @order_group_id, 'MPO-DEMO-2026-0001', @buyer_user_id, @farmer2_user_id,
       ...
       'Tran Thi Buyer', '0903234000', '123 Duong Demo, My An, Cao Lanh, Dong Thap',
       'Baseline COD delivered order for review demo.', 255000.00, 20000.00, 275000.00, @seed_now, @seed_now
...
SELECT @order_id, @marketplace_product_id, 'Ngo ngot Cao Nguyen Xanh Demo', 'ngo-ngot-cao-nguyen-xanh-demo', 'https://example.com/images/corn-demo.jpg',
...
SELECT @marketplace_product_id, @order_id, @order_item_id, @buyer_user_id, 5, 'Sản phẩm demo chất lượng tốt.', @seed_now, @seed_now, FALSE
